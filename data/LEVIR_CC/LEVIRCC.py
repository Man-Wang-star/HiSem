...
class LEVIRCCDataset(Dataset):
    ...
    def __init__(self, encoder_network, data_folder, list_path, split, token_folder = None, word_vocab = None, max_length = 41, allow_unk = 0, max_iters=None):
        """
    ...
    def __getitem__(self, index):
        ...
        if datafiles["token"] is not None:
            ...
            for j, tokens in enumerate(caption_list):
                nochange_cap = ['<START>', 'the', 'scene', 'is', 'the', 'same', 'as', 'before',
                                '<END>']
