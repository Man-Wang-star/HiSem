...
from model.hisem.model_encoder_MOE import Encoder, AttentiveEncoder
from model.hisem.model_decoder import DecoderTransformer
...
class Trainer(object):
    def __init__(self, args):
        ...
        print_log('=>encoder_dim: {}'.format(args.encoder_dim), self.log)
        print_log('=>embed_dim: {}'.format(args.embed_dim), self.log)
        print_log('=>fine_tune_encoder: {}'.format(args.fine_tune_encoder), self.log)
        ...
        self.log_var_cap = torch.nn.Parameter(torch.zeros(()))
        self.log_var_cls = torch.nn.Parameter(torch.zeros(()))
    ...
    def build_model(self):
        ...
        self.encoder_trans = AttentiveEncoder(n_layers=args.n_layers, feature_size=[args.feat_size, args.feat_size, args.encoder_dim],
                                              heads=args.n_heads, dropout=args.dropout, attention_module_kwargs={'top_k': args.top_k, 'num_experts': args.num_experts,
    ...
    def training(self, args, epoch):
        ...
        for id, batch_data in enumerate(self.train_loader):
            ...
            change_flag = batch_data['change_flag']
            imgA = imgA.cuda()
            imgB = imgB.cuda()
            token = token.cuda()
            token_len = token_len.cuda()
            change_flag = change_flag.cuda()

            # Forward prop.
            feat1, feat2 = self.encoder(imgA, imgB)
            if epoch <= 40:
                supervised_label = change_flag
            else:
                supervised_label = None
            feat, diff_feat, cls_logits = self.encoder_trans(feat1, feat2, supervised_label)  # [B,L,D]  [B,]
            ...
            if epoch <= self.warmup_epoch:
                cls_loss = self.criterion_cap_cls(cls_logits.detach(), change_flag)
            elif epoch <= self.num_epochs:
                # cosine ramp-up
                x = (epoch - self.warmup_epoch) / (self.num_epochs - self.warmup_epoch)
                alpha = 0.5 * (1 - math.cos(math.pi * x))
                cls_loss = self.criterion_cap_cls(cls_logits, change_flag)
                cls_loss = alpha * cls_loss
            else:
                cls_loss = self.criterion_cap_cls(cls_logits, change_flag)

            loss = cap_loss + 0.8 * cls_loss
            # Back prop.
    ...
    def validation(self, epoch):
        ...
        with torch.no_grad():
            ...
            for ind, batch_data in enumerate(
                ...
                if self.encoder is not None:
                    feat1, feat2 = self.encoder(imgA, imgB)
                feat, diff_feat, cls_logits = self.encoder_trans(feat1, feat2, supervised_label=None)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_folder',
                        default='/data1/wangman/RSCaMa-main/RSCaMa-main/data/LEVIR_CC/Levir-CC-dataset/images',
                        help='folder with data files')
    parser.add_argument('--list_path', default='/data1/wangman/RSCaMa-main/RSCaMa-main/data/LEVIR_CC/',
                        help='path of the data lists')
    parser.add_argument('--token_folder', default='/data1/wangman/RSCaMa-main/RSCaMa-main/data/LEVIR_CC/tokens/',
    ...
    parser.add_argument('--warmup_epoch', type=int, default=3, help='number of epochs to warmup train.')
    parser.add_argument('--gpu_id', type=int, default=0, help='gpu id in the training.')
    parser.add_argument('--checkpoint', default=None, help='path to checkpoint')
    parser.add_argument('--print_freq', type=int, default=100, help='print training/validation stats every __ batches')
    # Training parameters
    parser.add_argument('--fine_tune_encoder', type=bool, default=True, help='whether fine-tune encoder or not')   #True
    ...
    # backbone parameters
    parser.add_argument('--network', default='CLIP-ViT-B/16', help=' define the backbone encoder to extract features')
